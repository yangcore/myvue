        ...
        <el-row>
            <el-col :span="14"
                ...
                <el-input v-model="input"
                          v-if='showinpunt=="show"'></el-input>
                <span class="errorinfo"
                      v-if='efficacy'>可输入范围[0.00,99999999.99]的纯数字</span>
            </el-col>
            <el-col :span="4"
                    :push='2'>
                <el-button :type='inputtype'
                           @click='reviseMoney'>{{inputhtml}}</el-button>
            </el-col>
        </el-row>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        AlterInpunt() {
            this.$http.post('//bid.paicaifu.com/userAmount/resetAmount', {
                amount: this.input
            })
                .then((response) => {
